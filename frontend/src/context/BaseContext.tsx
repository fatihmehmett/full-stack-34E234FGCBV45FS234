...
interface BaseContextType {
  userData: DataType[];
  getUsers: (page:number|undefined,pageSize:number|undefined,search:string) => Promise<void>;
...
export const BaseProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const openMessage = (type: 'loading' | 'success' | 'error', content: string, duration: number = 2) => {
    messageApi.open({
      key,
      type,
      content,
  ...
  const getUsers = useCallback(async (page:number|undefined,pageSize:number|undefined,search:string) => {
  ...
  return (
    <BaseContext.Provider
      value={{ userData, getUsers, createUser, updateUser, deleteUser, pagination, searchText, setSearchText,searchOnClick }}
    >
      {contextHolder}
      {children}
    </BaseContext.Provider>
